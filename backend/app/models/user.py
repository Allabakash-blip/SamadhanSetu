...
class User(Base):
    ...
    account_status: Mapped[AccountStatus] = mapped_column(Enum(AccountStatus), default=AccountStatus.INCOMPLETE, nullable=False)
    ...
    industry_support_offers = relationship("IndustrySupportOffer", foreign_keys="IndustrySupportOffer.industry_id", cascade="all, delete-orphan")
    industry_partnerships = relationship("IndustryPartnership", foreign_keys="IndustryPartnership.industry_id", cascade="all, delete-orphan")
class State(Base):
...
class UniversityProfile(Base):
    ...
    university_type: Mapped[str | None] = mapped_column(String(100))
...
class IndustryProfile(Base):
    ...
    city: Mapped[str | None] = mapped_column(String(150))
    expertise: Mapped[str | None] = mapped_column(Text)
    available_support: Mapped[str | None] = mapped_column(Text)
    verification_status: Mapped[str] = mapped_column(String(30), default="PENDING")
...
class GovernmentProfile(Base):
    ...
    designation: Mapped[str | None] = mapped_column(String(150))
    official_id: Mapped[str | None] = mapped_column(String(100))
    state_id: Mapped[int | None] = mapped_column(ForeignKey("states.id"))
    district_id: Mapped[int | None] = mapped_column(ForeignKey("districts.id"))
    verification_status: Mapped[str] = mapped_column(String(30), default="PENDING")
