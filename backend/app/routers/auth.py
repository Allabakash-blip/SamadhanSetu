...
def google_login(
    ...
    if not google_id or not email:
        raise HTTPException(
            400,
            "Google did not provide required account information"
        )
...
def complete_profile(
    ...
    if (
        current_user.role
        and current_user.role != UserRole(role)
    ):
        raise HTTPException(
            400,
    ...
    # ---------------------------------------------------------
        ...
        profile = (
            current_user.university_profile
            or UniversityProfile(
                user_id=current_user.id,
